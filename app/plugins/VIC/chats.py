...
def extend_or_add(message: Message, data: list):
    if message.unique_chat_user_id in CONVO_DICT:
        CONVO_DICT[message.unique_chat_user_id].extend(data)
    else:
...
async def text_query(bot: BOT, message: Message | Msg):
    ...
    history = CONVO_DICT.get(message.unique_chat_user_id, [])
...
async def get_media_list(message: Message) -> list[dict[str, str]]:
    if message.media_group_id:
        group = CACHED_MEDIA_GROUPS.pop(message.media_group_id)
...
async def get_media_group(message: Message) -> list[Message]:
    ...
    cache = CACHED_MEDIA_GROUPS.get(media_id)
    if cache:
        return cache
    group = await message.get_media_group()
    CACHED_MEDIA_GROUPS[media_id] = group
    return group
