...
from vic.chat_queries.text_query import text_query
...
@bot.on_message(filters.command(commands="start", prefixes="/"), group=2)
...
async def ask_query(bot: BOT, message: Msg):
    ...
    history = extra_config.CONVO_DICT.pop(message.unique_chat_user_id, None)
    if history:
...
async def total_chats(bot: BOT, message: Message):
    users = len(extra_config.CHATS["PRIVATE"])
    groups = len(extra_config.CHATS["GROUPS"])
    await message.reply(
        f"<u>BOT STATS</u>\n<b>Users</b>: {users}\n<b>Chats</b>: {groups}"
    )
...
async def broadcast_text(bot: BOT, message: Message):
    if not message.input:
        ...
        return
    chats = extra_config.CHATS["GROUPS"]
    total = len(chats)
    total_failed = 0
    failed_str = ""
    for chat in chats:
